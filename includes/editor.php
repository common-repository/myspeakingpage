...
function msp_editor_init() {
	if(is_admin() and msp_is_editor_page()) {
		...
		add_action('admin_init', 'msp_add_admin_notices', 20);
		add_action('admin_body_class', 'msp_add_admin_body_class', 20);
	}
...
function msp_add_admin_notices() {
...
function msp_add_admin_body_class($classes) {
	$classes .= ' msp-admin-page';
	return $classes;
}
...
function msp_enqueue_editor_resources() {
	...
	wp_enqueue_script('msp-spectrum', plugins_url('js/spectrum.js', dirname(__FILE__)), array('jquery'), MSP_VERSION);
	wp_enqueue_script('msp-editor-script', plugins_url('js/editor.js', dirname(__FILE__)), array('jquery', 'jquery-ui-core', 'jquery-ui-widget', 'jquery-ui-sortable', 'msp-spectrum'), MSP_VERSION);
	wp_enqueue_style('msp-editor-style', plugins_url('css/editor.css', dirname(__FILE__)), array(), MSP_VERSION);
	wp_enqueue_style('msp-spectrum-style', plugins_url('css/spectrum.css', dirname(__FILE__)), array(), MSP_VERSION);
}
...
?>
	...
	<div class="msp-section-field">
		<label class="msp-section-field-title" for="msp_button_color">Button Color</label>
		<input type="text" name="msp_button_color" id="msp_button_color" value="<?php echo(get_post_meta($post->ID, "msp_button_color", true)); ?>"/>
		<p class="description">Choose the color of the button.</p>
	</div>
<?php
}
...
function msp_save_editor_metaboxes($post_id) {
	...
	if($post_id == msp_get_setting('speaking_page')) {
		...
		if(isset($_REQUEST['msp_button_color'])) { update_post_meta($post_id, 'msp_button_color', $_REQUEST['msp_button_color']); }
