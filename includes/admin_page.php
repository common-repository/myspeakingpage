...
function msp_enqueue_admin_resources() {
	wp_enqueue_style('msp-admin-style', plugins_url('css/admin.css', dirname(__FILE__)), array(), MSP_VERSION);
}
...
function msp_email_subscribe_notice() {
	...
	$content .= '<p>'.__('Join over 7,000 other speakers and authors on the Author Media&#39;s award winning newsletter. AuthorMedia.com has been frequently recommended by Writer&#39;s Digest as one of the most helpful websites for authors. You can unsubscribe at anytime with just one click.', 'myspeakingpage').'</p>';
	...
	<script type="text/javascript">
		...
		jQuery(document).ready(function () {
			...
			jQuery('#msp-pointer-no').click(function() {
				jQuery.post(ajaxurl, {action: 'msp_email_subscribe_notice', subscribe: 'no'});
				jQuery('#wpadminbar').pointer('close');
			});
