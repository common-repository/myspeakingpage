...
function msp_frontend_init() {
	if(!is_admin()) {
		...
		add_action('wp_enqueue_scripts', 'msp_enqueue_frontend_resources');
		add_action('add_admin_bar_menus', 'msp_override_admin_bar');
	}
	add_image_size('msp_headshot', 1000, 100, false);
	add_filter('msp_content', 'wptexturize');
	add_filter('msp_content', 'convert_smilies');
	add_filter('msp_content', 'convert_chars');
	add_filter('msp_content', 'wpautop');
	add_filter('msp_content', 'shortcode_unautop');
	add_filter('msp_content', 'do_shortcode');
}
...
function msp_enqueue_frontend_resources() {
	wp_enqueue_style('msp-frontend-style', plugins_url('css/frontend.css', dirname(__FILE__)), array(), MSP_VERSION);
	add_action('wp_head', 'msp_add_custom_css');
}

function msp_add_custom_css() {
	global $post;
	if(!empty($post) and $post->ID == msp_get_setting('speaking_page')) {
		$button_color = get_post_meta($post->ID, 'msp_button_color', true);
		if($button_color) {
			echo('<style type="text/css">');
			echo('.myspeakingpage a.msp-button { background-color: '.$button_color.' !important; } ');
			echo('</style>');
		}
	}
}
...
function msp_get_breadcrumbs() {
	...
	return apply_filters('msp_filter_breadcrumbs', empty($output) ? '' : '<div class="msp-breadcrumbs">'.$output.'</div>');
}
...
function msp_override_page_content($content) {
	...
	if(!empty($post) and $post->ID == msp_get_setting('speaking_page')) {
		if(!empty($_REQUEST['msp_page']) and $_REQUEST['msp_page'] == 'testimonials') {
			...
			return apply_filters('msp_testimonials_page_content', $output);
		}
		if(!empty($_REQUEST['msp_page']) and $_REQUEST['msp_page'] == 'topics') {
			$output .= '<div class="myspeakingpage">';
			...
			return apply_filters('msp_topics_page_content', $output);
		}
		...
		foreach($sections as $section) {
			if($section == 'msp_action_image_metabox') {
				...
				$output .= apply_filters('msp_content', $post->post_excerpt);
				$output .= '</section>';
				...
				$output .= apply_filters('msp_content', $post->post_content);
				$output .= '</section>';
					...
					if(!empty($num_upcoming_events)) {
						...
						if(is_array($events->posts) and count($events->posts) > 0) {
							foreach($events->posts as $event) {
								$location = mse_get_event_location($event->ID);
								$time = mse_get_event_time_start($event->ID, 'm/d/Y');
								$timeandloc = ($time and $location) ? $location.', '.$time : ($time ? $time : ($location ? $location : ''));
								$output .= '<div class="msp-event">';
								$output .= '<a href="'.get_permalink($event->ID).'" class="msp-event-title">'.$event->post_title.'</a>';
								$output .= $timeandloc ? ' - '.$timeandloc : '';
								$output .= '</div>';
							}
		...
		return apply_filters('msp_page_content', $output);
	}
